            ...
            if (!(physical.Layer == Layer.Projectile))
            {
                ...
                foreach (Physical neighbor in Physicals)
                {
                    ...
                    if (distance < touch_distance)
                        physical.Force += 100 * 
                                          neighbor.Mass * 
                                          (physical.Position - neighbor.Position).normalized * 
                                          (touch_distance - distance) / touch_distance;
                }
                ...
                if (physical.IsTouchingTerrain)
                {
                    ...
                    physical.Roll = Mathf.LerpAngle(
                        physical.Roll, 
                        MathUtility.RadiansToDegrees(target_roll), 5 * Time.deltaTime);
                }
            }
